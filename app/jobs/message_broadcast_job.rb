...
class MessageBroadcastJob < ApplicationJob
  ...
  def perform(message)
    ...
    ActionCable.server.broadcast "conversation_#{message.by_id}_channel", message: render_message(message, true)
    ActionCable.server.broadcast "conversation_#{message.for_id}_channel", message: render_message(message, false)
  end
